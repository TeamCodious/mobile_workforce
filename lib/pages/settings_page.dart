...
class SettingsPage extends HookWidget {
                  ...
                  onPressed: () async {
                    await showDialog(
                        ...
                        builder: (BuildContext context) {
                          return AlertDialog(
                            title: Text(
                                "Are you sure to take a break for a while?"),
                            actions: [
                              FlatButton(
                                  onPressed: () {
                                    Navigator.of(context).pop();
                                    ...
                                    if (dateTime.isAfter(next30)) {
                                      ...
                                      await showDialog(
                                          ...
                                          builder: (context) {
                                            return AlertDialog(
                                              title: Text(
                                                  'You are not allowed to take a break longer than 30 minute.'),
                                              actions: [
                                                FlatButton(
                                                    onPressed: () {
                                      ...
                                      print(
                                          "[Pref]: ${pref.getInt(Global.BREAK_TIME)}");
                                      print(
                                          "[Minute]: ${dateTime.difference(now).inMinutes}");
                                      breakTime(id,
                                          dateTime.difference(now).inMinutes);
                                      Global.setBreak();
  ...
  void stop() async {
    ...
    String body =
        '{"id": "$id", "stop_time": ${DateTime.now().toUtc().millisecondsSinceEpoch}}';
  ...
  void breakTime(String id, int time) async {
    ...
    String body =
        '{"id": "$id", "total_break": $time, "now": ${DateTime.now().toUtc().millisecondsSinceEpoch}}';
    Response res = await patch(url, headers: Global.HEADERS, body: body);
    if (res.statusCode == 204) {
      print('done');
    } else {
  ...
  void createTime() async {
    ...
    String body =
        '{"id": "$id", "employee": "${CurrentUserId.id}", "start_time": ${DateTime.now().toUtc().millisecondsSinceEpoch}}';
    print(body);
    Response res = await put(url, headers: Global.HEADERS, body: body);
    if (res.statusCode == 201) {
      print('done');
